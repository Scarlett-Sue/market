  ...
  <div class="flow-wrap">
    <el-card class="my-card">
      <div slot="header" class="clearfix">
        <span style="color: #2e343a;">经销商管理</span>
      </div>
      <div class="form">
...
import baseInfo from '../../components/dealer/baseInfo';
export default {
  components: {
    baseInfo,
  },
  ...
  async mounted() {
    ...
    if (this.id) {
      let detail = await manage.dealerDetail({ id: this.id });
      this.detail = detail;
    }
    load.close();
  },
  methods: {
    async submit(operate) {
      ...
      try {
        if (this.id) {
          let res = await manage.dealerUpdate(infoForm);
          ...
          let res = await manage.dealerAdd(infoForm);
          this.btnLoading = false;
          if (res.code === '20000') {
